...
import type { PriceSummary, InventorySummary } from "@/hooks/use-prices"
import { COMMODITY_INFO, type CommodityCode } from "@/db/schema-commodity"
...
function CommoditySelector({
  ...
  const displayName = (info: typeof current) => (lang === "en" ? info.englishName : info.name)
            ...
            {items.map(([code, info]) => (
              ...
              >
                ...
                <div className="flex-1 text-left">
                  <div className="font-medium">{displayName(info)}</div>
...
// 数据值 → 翻译 key 映射（计算值仍为中文代码，展示时翻译）
const heatKeyMap: Record<string, string> = {
  "旺盛": "dashboard.heat.hot",
  "活跃": "dashboard.heat.active",
  "温和": "dashboard.heat.mild",
  "暂无数据": "dashboard.heat.noData",
}
const heatLabelKeyMap: Record<string, string> = {
  "需求旺盛": "dashboard.heatLabel.hot",
  "供需平衡": "dashboard.heatLabel.balanced",
  "需求一般": "dashboard.heatLabel.general",
  "待接入库存数据": "dashboard.heatLabel.noData",
}
const riskKeyMap: Record<string, string> = {
  "高": "dashboard.riskHigh",
  "中等": "dashboard.riskMedium",
  "低": "dashboard.riskLow",
}
const riskLabelKeyMap: Record<string, string> = {
  "波动较大": "dashboard.riskLabel.volatile",
  "关注走势": "dashboard.riskLabel.watch",
  "相对稳定": "dashboard.riskLabel.stable",
}
const trendKeyMap: Record<string, string> = {
  "上涨": "dashboard.trend.up",
  "下跌": "dashboard.trend.down",
  "震荡": "dashboard.trend.flat",
}
...
function ReportCarouselInline() {
  ...
  if (isLoading) {
    return <div className="animate-pulse text-slate-400 text-sm py-4">{t("common.loading")}</div>
  }

  if (reports.length === 0) {
    return <div className="text-slate-400 text-sm py-4">{t("reports.noReports")}</div>
  }
    ...
    <div className="flex flex-col h-full min-h-0 w-full">
      <div className="p-2 rounded-lg bg-white/80 dark:bg-white/10 border border-slate-200/80 dark:border-white/10 flex-1 flex flex-col min-h-0 w-full">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className={`text-xs px-1.5 py-0.5 rounded shrink-0 ${getRiskColor(currentReport.riskLevel)}`}>
            {t(riskKeyMap[currentReport.riskLevel || "低"] || currentReport.riskLevel || "低")}
          </span>
        </div>
      ...
      <div className="flex items-center justify-between mt-2 px-1">
        ...
        <Link href="/reports" className="text-xs text-cyan-600 dark:text-cyan-400 flex items-center gap-0.5 hover:text-cyan-700">
          {t("dashboard.more")} <ChevronRight className="h-3 w-3" />
        </Link>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-slate-900 dark:text-white">{t("dashboard.enterprisePricePredictions")}</h3>
        <Link href="/enterprises" className="text-xs text-cyan-600 dark:text-cyan-400 flex items-center gap-0.5 hover:text-cyan-700">
          {t("dashboard.details")} <ChevronRight className="h-2.5 w-2.5" />
        </Link>
      </div>
      ...
      <div className="mt-3 pt-3 border-t border-slate-200 dark:border-white/10">
        <p className="text-sm text-slate-500 dark:text-slate-400">{t("dashboard.avgPredictedPricePrefix")}<span className="font-medium text-cyan-600 dark:text-cyan-400">{avgPrice}</span>{t("dashboard.perTon")}</p>
        {enterprises[0]?.source && (
          <p className="text-[10px] text-slate-300/70 dark:text-slate-600/70 mt-0.5">{t("dashboard.sourcePrefix")}{enterprises[0].source}</p>
        )}
      </div>
    </div>
  )
...
export function DashboardContent({ initialData }: { initialData: DashboardData | null }) {
  ...
  // 单一 API 端点获取仪表盘全部数据（替代 3 个独立请求）
  const dashboardQuery = useQuery({
    queryKey: ["dashboard", commodity],
    queryFn: async () => {
      const res = await fetch(`/api/dashboard?commodity=${commodity}`)
      const json = await res.json()
      return json.data as DashboardData
    },
    // Server Component 预取的数据作为初始值，首次渲染秒出
    initialData: commodity === "sulfur" ? initialData : undefined,
  })

  // 从合并 API 数据计算展示值
  const data = useMemo(() => {
    const d = dashboardQuery.data
    const priceData = d?.priceSummary
    const invData = d?.inventorySummary
    const epData = d?.enterprisePredictions || []
    ...
    if (priceData?.currentPrice) {
      ...
      // 根据真实数据动态生成市场洞察
      const name = lang === "en" ? COMMODITY_INFO[commodity].englishName : COMMODITY_INFO[commodity].name
      const insightParts: string[] = []
      if (changePercent > 2) insightParts.push(lang === "en" ? `${name} price rose ${changePercent.toFixed(1)}% recently; watch cost pressure.` : `${name}价格近期上涨${changePercent.toFixed(1)}%，需关注成本压力。`)
      else if (changePercent < -2) insightParts.push(lang === "en" ? `${name} price dropped ${Math.abs(changePercent).toFixed(1)}% recently; favorable buying window.` : `${name}价格近期下跌${Math.abs(changePercent).toFixed(1)}%，采购窗口有利。`)
      else insightParts.push(lang === "en" ? `${name} price is relatively stable; purchase as needed.` : `${name}价格近期相对平稳，按需采购为主。`)
      if (inventoryLevel) insightParts.push(lang === "en" ? `Port inventory is about ${(inventoryLevel / 10000).toFixed(1)} ten-thousand tons.` : `当前港口库存约${(inventoryLevel / 10000).toFixed(1)}万吨。`)
      if (changePercent > 3) insightParts.push(lang === "en" ? "Watch downstream operating rates and international supply; control purchasing pace." : "建议关注下游开工率及国际供应动态，控制采购节奏。")
      else if (changePercent < -3) insightParts.push(lang === "en" ? "Consider increasing purchase volume to lock in low-cost inventory." : "可适当增加采购量，锁定低成本库存。")

      // 计算数据新鲜度
      let freshnessCode: "today" | "oneDay" | "days" | "unknown" = "unknown"
      let daysAgo = 0
      if (priceData?.date) {
        ...
        daysAgo = diffDays
        if (diffDays === 0) freshnessCode = "today"
        else if (diffDays === 1) freshnessCode = "oneDay"
        else freshnessCode = "days"
      }
      const priceSource = priceData?.source || (lang === "en" ? "Unknown source" : "未知来源")
      const invSource = invData?.source || (lang === "en" ? "No inventory data" : "暂无库存数据")

      return {
        avgPrice: `¥${currentPrice.toLocaleString()}`,
        priceLabel: name + (priceData.market ? ` ${priceData.market}` : t("dashboard.spotAvg")),
    ...
    // API 数据不可用时返回空状态
    return null
  }, [commodity, dashboardQuery.data, t, lang])

  const isLoading = dashboardQuery.isLoading
  const isError = dashboardQuery.isError

  // 切换品种时缓存上次有效数据，保持局部加载而非整页替换
  const [displayData, setDisplayData] = useState<typeof data>(null)
  useEffect(() => {
    if (data) setDisplayData(data)
  }, [data])
  const isTransitioning = !data && !!displayData && isLoading
      ...
      <div className="relative px-3 pt-3 pb-3 max-w-full" suppressHydrationWarning>
          ...
          <div className="flex-1 flex items-center justify-center">
            {isLoading ? (
              <div className="text-center">
                ...
                <p className="text-sm text-slate-500">{t("dashboard.loadingMarket")}</p>
              </div>
            ) : isError ? (
              <div className="text-center">
                <AlertTriangle className="h-8 w-8 text-amber-500 mx-auto mb-2" />
                <p className="text-sm text-slate-500">{t("dashboard.loadFailed")}</p>
                <button
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-slate-500">{t("dashboard.noCommodityDataPrefix")}{lang === "en" ? COMMODITY_INFO[commodity].englishName : COMMODITY_INFO[commodity].name}{t("dashboard.noCommodityDataSuffix")}</p>
              </div>
            )}
          </div>
        )}
        ...
        {isTransitioning && (
          <div className="flex items-center gap-2 mb-3 px-1">
            ...
            <span className="text-xs text-slate-400">{t("dashboard.refreshingPrefix")}{lang === "en" ? COMMODITY_INFO[commodity].englishName : COMMODITY_INFO[commodity].name}{t("dashboard.refreshingSuffix")}</span>
          </div>
        )}
        {/* 统计概览四卡片 - 平铺整行 */}
        <div className={`grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3 transition-opacity duration-200 ${isTransitioning ? "opacity-60" : ""}`}>
          {/* 当前均价 */}
          <div className={`bg-gradient-to-br ${colors.card} backdrop-blur-sm rounded-lg p-3 border ${colors.border}`}>
            <div className="flex items-center justify-between mb-0.5">
              <span className="text-sm text-slate-500 dark:text-slate-400">{t("dashboard.avgPrice")}</span>
            ...
            <p className="text-[10px] mt-0.5 flex items-center gap-1">
              <span className={`px-1 py-0.5 rounded font-medium ${
                ...
                {displayData.priceSource.includes("模拟") ? t("dashboard.simulatedData") : displayData.priceSource.includes("推算") ? t("dashboard.modelDerived") : t("dashboard.realData")}
              </span>
              <span className="text-slate-400/80 dark:text-slate-500/80 truncate">{t("dashboard.sourcePrefix")}{displayData.priceSource}</span>
            </p>
          </div>
          ...
          <div className={`bg-gradient-to-br ${colors.card} backdrop-blur-sm rounded-lg p-3 border ${colors.border}`}>
            <div className="flex items-center justify-between mb-0.5">
              <span className="text-sm text-slate-500 dark:text-slate-400">{t("dashboard.trend")}</span>
            ...
            <span className="text-lg font-bold text-slate-900 dark:text-white">{t(trendKeyMap[displayData.trend] || displayData.trend)}</span>
            <div className="flex items-center gap-0.5 mt-0.5">
              <Activity className={`h-2.5 w-2.5 ${colors.icon}`} />
              <span className="text-xs text-slate-500">{t("dashboard.trend30d")}</span>
            </div>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{t("dashboard.monthlyTrendJudgment")}</p>
            <p className="text-[10px] text-slate-300/70 dark:text-slate-600/70 mt-0.5">{t("dashboard.sourcePrefix")}{displayData.priceSource} {t("dashboard.priceSeriesCalc")}</p>
          </div>

          {/* 市场热度 */}
          <div className={`bg-gradient-to-br ${colors.card} backdrop-blur-sm rounded-lg p-3 border ${colors.border}`}>
            <div className="flex items-center justify-between mb-0.5">
              <span className="text-sm text-slate-500 dark:text-slate-400">{t("dashboard.heat")}</span>
              <Zap className={`h-4 w-4 ${colors.icon}`} />
            </div>
            <span className={`text-lg font-bold ${displayData.marketHeat === "暂无数据" ? "text-slate-400 dark:text-slate-500" : "text-slate-900 dark:text-white"}`}>{t(heatKeyMap[displayData.marketHeat] || displayData.marketHeat)}</span>
            <div className="flex items-center gap-0.5 mt-0.5">
              ...
              <span className="text-xs text-slate-500">{t(heatLabelKeyMap[displayData.marketHeatLabel] || displayData.marketHeatLabel)}</span>
            </div>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{t("dashboard.demandActivity")}</p>
            <p className="text-[10px] text-slate-300/70 dark:text-slate-600/70 mt-0.5">{t("dashboard.sourcePrefix")}{displayData.invSource}</p>
          </div>

          {/* 风险等级 */}
          <div className={`bg-gradient-to-br ${colors.card} backdrop-blur-sm rounded-lg p-3 border ${colors.border}`}>
            <div className="flex items-center justify-between mb-0.5">
              <span className="text-sm text-slate-500 dark:text-slate-400">{t("dashboard.risk")}</span>
              <AlertTriangle className={`h-4 w-4 ${colors.icon}`} />
            </div>
            <span className={`text-lg font-bold ${displayData.risk === "高" ? "text-rose-600 dark:text-rose-400" : displayData.risk === "中等" ? "text-amber-600 dark:text-amber-400" : "text-emerald-600 dark:text-emerald-400"}`}>{t(riskKeyMap[displayData.risk] || displayData.risk)}</span>
            <div className="flex items-center gap-0.5 mt-0.5">
              <Target className={`h-2.5 w-2.5 ${colors.icon}`} />
              <span className="text-xs text-slate-500">{t(riskLabelKeyMap[displayData.riskLabel] || displayData.riskLabel)}</span>
            </div>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{t("dashboard.riskAssessment")}</p>
            <p className="text-[10px] text-slate-300/70 dark:text-slate-600/70 mt-0.5">{t("dashboard.ruleBased30d")}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-3 px-1">
          <div className="flex items-center gap-2">
            <span className={`w-1.5 h-1.5 rounded-full ${displayData.freshnessCode === "today" ? "bg-emerald-400" : "bg-amber-400"}`} />
            <span className="text-[11px] text-slate-400 dark:text-slate-500">
              {t("dashboard.dataUpdatedPrefix")}
              {displayData.freshnessCode === "today" ? t("dashboard.freshness.today") : displayData.freshnessCode === "oneDay" ? t("dashboard.freshness.oneDayAgo") : displayData.freshnessCode === "days" ? `${displayData.daysAgo}${t("dashboard.freshness.daysAgoSuffix")}` : t("dashboard.freshness.unknown")}
            </span>
          </div>
          <span className="text-[11px] text-slate-400/60 dark:text-slate-500/60">
            {t("dashboard.priceSourcePrefix")}{displayData.priceSource} | {t("dashboard.invSourcePrefix")}{displayData.invSource}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
          ...
          <Link href="/market-analysis?tab=commodities" className="rounded-lg border border-slate-200/60 dark:border-white/10 bg-white/60 dark:bg-white/5 p-2.5 hover:border-cyan-300/50 dark:hover:border-cyan-500/30 transition-all group cursor-pointer">
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <span className="text-xs text-slate-500">{t("dashboard.commodityCoverage")}</span>
            </div>
            <div className="text-lg font-bold text-slate-900 dark:text-white">4<span className="text-sm font-normal text-slate-400 ml-1">{t("dashboard.varietiesUnit")}</span></div>
            <div className="flex items-center gap-1 mt-0.5">
              ...
              <span className="text-[10px] text-slate-400">{t("dashboard.commodityList")}</span>
            </div>
          </Link>
          ...
          <Link href="/market-analysis?tab=accuracy" className="rounded-lg border border-slate-200/60 dark:border-white/10 bg-white/60 dark:bg-white/5 p-2.5 hover:border-emerald-300/50 dark:hover:border-emerald-500/30 transition-all group cursor-pointer">
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <span className="text-xs text-slate-500">{t("dashboard.modelAccuracy")}</span>
            </div>
            ...
            <div className="flex items-center gap-1 mt-0.5">
              ...
              <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium">{t("dashboard.excellent")}</span>
            </div>
          </Link>
          ...
          <Link href="/api-keys" className="rounded-lg border border-slate-200/60 dark:border-white/10 bg-white/60 dark:bg-white/5 p-2.5 hover:border-amber-300/50 dark:hover:border-amber-500/30 transition-all group cursor-pointer">
            ...
            <div className="text-lg font-bold text-slate-900 dark:text-white">7<span className="text-sm font-normal text-slate-400 ml-1">{t("dashboard.countUnit")}</span></div>
            <div className="flex items-center gap-1 mt-0.5">
              <span className="text-[10px] text-slate-400">{t("dashboard.mcpToolsDesc")}</span>
            </div>
          </Link>
          ...
          <Link href="/enterprises" className="rounded-lg border border-slate-200/60 dark:border-white/10 bg-white/60 dark:bg-white/5 p-2.5 hover:border-blue-300/50 dark:hover:border-blue-500/30 transition-all group cursor-pointer">
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <span className="text-xs text-slate-500">{t("dashboard.enterpriseAccess")}</span>
            </div>
          ...
          <div className="space-y-3 flex flex-col min-h-0">
            ...
            <div className="rounded-lg p-2.5 bg-white/50 dark:bg-white/3 border border-slate-200/50 dark:border-white/5">
              <div className="flex items-center gap-2 mb-2 px-1">
                ...
                <span className="text-xs font-medium text-slate-500 dark:text-slate-400">{t("dashboard.commodityCoverage")}</span>
                <span className="text-[10px] px-1 py-0.5 rounded bg-cyan-100 dark:bg-cyan-500/20 text-cyan-600 dark:text-cyan-400">{t("dashboard.expanding")}</span>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className={`text-xs font-semibold ${isActive ? styles.text : "text-slate-500 dark:text-slate-400"}`}>
                        {lang === "en" ? info.englishName : info.name}
                      </div>
                      <div className={`text-[10px] mt-0.5 ${isActive ? styles.subtext : "text-slate-400 dark:text-slate-500"}`}>
                        {isActive ? <span className="inline-flex items-center gap-0.5"><span className="w-1 h-1 rounded-full bg-emerald-400 animate-pulse" />{t("dashboard.selected")}</span> : t("dashboard.switch")}
                      </div>
                    </button>
                  )
                })}
            ...
            <div className={`bg-gradient-to-br ${colors.card} backdrop-blur-sm rounded-lg p-3 border ${colors.border}`}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-semibold text-slate-900 dark:text-white">{t("dashboard.marketInsight")}</h3>
              </div>
            ...
            <div className="bg-white/80 dark:bg-white/5 backdrop-blur-sm rounded-lg p-3 border border-slate-200 dark:border-white/10 shadow-sm h-[340px] flex-shrink-0 flex flex-col">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-slate-900 dark:text-white font-medium text-sm">{t("dashboard.priceGraph")}</h3>
                </div>
              ...
              <div className="relative flex-1 min-h-0 rounded-lg bg-gradient-to-br from-slate-100 via-slate-50 to-slate-100 dark:from-slate-900/50 dark:via-slate-800/30 dark:to-slate-900/50 border border-slate-200 dark:border-white/5 overflow-hidden">
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" style={{ zIndex: 10 }}>
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-cyan-200 via-blue-100 to-cyan-100 dark:from-cyan-500/50 dark:via-blue-500/40 dark:to-cyan-500/50 border-2 border-cyan-500 dark:border-cyan-400/70 flex items-center justify-center shadow-md" style={{ animationDuration: '2s' }}>
                    <div className="text-center">
                      ...
                      <span className="text-xs text-slate-800 dark:text-white font-bold">{t("dashboard.graph.price")}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute top-[6%] left-1/2 -translate-x-1/2" style={{ zIndex: 10 }}>
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-emerald-200 to-green-100 dark:from-emerald-500/50 dark:to-green-500/40 border border-emerald-500 dark:border-emerald-400/60 flex items-center justify-center shadow-sm">
                    <div className="text-center">
                      ...
                      <span className="text-[9px] text-slate-800 dark:text-white font-bold">{t("dashboard.graph.supply")}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute bottom-[6%] left-1/2 -translate-x-1/2" style={{ zIndex: 10 }}>
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-violet-200 to-purple-100 dark:from-violet-500/50 dark:to-purple-500/40 border border-violet-500 dark:border-violet-400/60 flex items-center justify-center shadow-sm">
                    <div className="text-center">
                      ...
                      <span className="text-[9px] text-slate-800 dark:text-white font-bold">{t("dashboard.graph.demand")}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute top-1/2 left-[10%] -translate-y-1/2" style={{ zIndex: 10 }}>
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-amber-200 to-orange-100 dark:from-amber-500/50 dark:to-orange-500/40 border border-amber-500 dark:border-amber-400/60 flex items-center justify-center shadow-sm">
                    <div className="text-center">
                      ...
                      <span className="text-[9px] text-slate-800 dark:text-white font-bold">{t("dashboard.graph.cost")}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute top-1/2 right-[10%] -translate-y-1/2" style={{ zIndex: 10 }}>
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-rose-200 to-pink-100 dark:from-rose-500/50 dark:to-pink-500/40 border border-rose-500 dark:border-rose-400/60 flex items-center justify-center shadow-sm">
                    <div className="text-center">
                      ...
                      <span className="text-[9px] text-slate-800 dark:text-white font-bold">{t("dashboard.graph.policy")}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute bottom-2 right-2 px-2 py-1 rounded bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm border border-slate-300 dark:border-slate-600 text-[10px] text-slate-500 dark:text-slate-400" style={{ zIndex: 15 }}>
                  <div className="flex items-center gap-1">
                    ...
                    <span>{t("dashboard.energyFlow")}</span>
                  </div>
                </div>
              </div>
            </div>
