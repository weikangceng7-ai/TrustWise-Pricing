...
export function registerCrossCommodityTools(
  server: McpServer,
  _config: McpConfig,
  client: ReturnType<typeof createClient>
) {
  server.tool(
    "cross_commodity_analysis",
    "当用户询问品种对比、哪些品种相关、一起采购怎么省钱、多品种协同采购、相关性分析时使用。跨品种大宗原料对比分析，返回价格对比、品种间相关性矩阵和协同采购建议",
    {},
    async (
      {},
      ...
      try {
        ...
        const commodities = data.commodities as Array<Record<string, unknown>> | undefined
        const correlations = data.correlations as Array<Record<string, unknown>> | undefined
        ...
        if (correlations && correlations.length > 0) {
          ...
          for (const c of correlations) {
            const corrIcon = (c.value as number) > 0.5 ? "🔴 强正相关" : (c.value as number) > 0 ? "🟡 弱正相关" : (c.value as number) < -0.3 ? "🟢 负相关" : "⚪ 不相关"
