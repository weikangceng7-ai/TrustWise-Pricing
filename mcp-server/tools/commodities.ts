  ...
  server.tool(
    ...
    async (
      ...
      try {
        ...
        const commodities = (result.data as any)?.commodities || (result.data as any)?.data || []
        ...
        for (const c of commodities) {
          ...
          if (c.price) lines.push(`   当前价格: ¥${c.price} | 涨跌: ${c.change || "持平"}`)
          lines.push(`   波动性: ${c.volatility || "N/A"} | 风险等级: ${c.riskLevel || "N/A"} | 市场热度: ${c.marketHeat || "N/A"}`)
          if (c.outlook) lines.push(`   展望: ${c.outlook}`)
          lines.push("")
        }
  ...
  server.tool(
    ...
    async (
      ...
      try {
        ...
        const analysis = (result.data as any)?.analysis || (result.data as any)
        if (!analysis) {
          return {
            content: [
