...
export function registerGetInventory(
  server: McpServer,
  config: McpConfig,
  ...
  server.tool(
    "get_inventory",
    `当用户询问港口库存、库存水平、库存变化、港口存货情况时使用。获取${config.INDUSTRY_CODE === "sulfur" ? "港口硫磺" : "行业"}库存数据，包括当前库存量和环比变化`,
    {
    ...
    async (
      ...
      try {
        ...
        const inventory = (result.data as any)?.data?.inventory || (result.data as any)?.inventory || []
